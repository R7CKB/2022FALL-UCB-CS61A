def flatten(s):
    ...
    "*** YOUR CODE HERE ***"
    '''作弊方法'''
    # return [int(x) for x in str(s) if x != '[' and x != ',' and x != ']' and x != ' ']
    '''官方方法(使用递归)'''
    if not s:
        return []
    elif type(s[0]) == list:
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    close_city=make_city('close_city',lat,lon)
    distance1,distance2=distance(close_city,city_a),distance(close_city,city_b)
...
def berry_finder(t): # bingo！
    ...
    if label(t) == 'berry':
        return True
    for x in branches(t):# cheated
        if berry_finder(x): # 因为每个树枝都是树，所以可以用这个，这种方法很难想到，思考了半天也没有想出来
            return True
    return False
...
def preorder(t):
    ...
    list1 = [label(t)]
    for b in branches(t):
        list1.extend(preorder(b))
    return list1
    '''官方答案''' # 思路是一样的，我的可能会更加简洁一点
    if branches(t) == []:
        return [label(t)]
    flattened_branches = []
...
def add_trees(t1, t2):
    ...
    "*** YOUR CODE HERE ***"
    '''自己写的(特别臃肿)'''
    add_tree = [label(t1) + label(t2)] # 这种情况就包含了叶子的情况
    for i in range(max(len(branches(t1)), len(branches(t2)))):
        if i < min(len(branches(t1)), len(branches(t2))):
            add_tree.append(add_trees(branches(t1)[i], branches(t2)[i]))
        elif len(branches(t1)) > len(branches(t2)):
            add_tree.append(branches(t1)[i])
        elif len(branches(t1)) < len(branches(t2)):
            add_tree.append(branches(t2)[i])
    return add_tree
    '''官方答案(思路好像是一样的)'''
