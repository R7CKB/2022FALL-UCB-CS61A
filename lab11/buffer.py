...
class Buffer:
    ...
    def pop_first(self):
        ...
        # cheated
        "*** YOUR CODE HERE ***"
        # Current和pop_first思路是一样的
        # 两种方案 
        # first solution
        # if self.more_on_line():
        #     token = self.current_line[self.index]
        #     self.index += 1 
        #     return token
        # else:
        #     self.current()
        ...
        # second solution
        if not self.more_on_line():# 如果索引不小于列表的长度
            try:
    ...
    def current(self):
        ...
        while not self.more_on_line():# 只要索引不小于列表的长度
            try:
                # BEGIN PROBLEM 1
                "*** YOUR CODE HERE ***"
                self.current_line=next(self.source)
    ...
    def more_on_line(self):
        return self.index < len(self.current_line) # 这里就应该想到它(self.current_line)是一个列表了
